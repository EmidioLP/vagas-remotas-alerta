...
distinguem presencial de hibrido). Copiado do projeto irmao vagas-tech-junior. As regras vivem em `scraper/rules/modalidade.yml`
-- edite la, nao aqui.
"""
...
def completar_modalidade(jobs: list[Job], inferidor: InferidorModalidade | None = None) -> list[Job]:
    ...
    Roda no pipeline antes do filtro de remotas, e nos pre-filtros das fontes
    que buscam a descricao vaga a vaga (so com titulo e local, nessa altura).
    """
