...
O `robots.txt` (`Allow: /`) aponta o `sitemap.xml`, e o caminho aqui e o mesmo
da GeekHunter: sitemap -> filtro pelo slug -> pagina da vaga.

Medido antes de virar codigo (22/09/2026):

  - o sitemap tem 3,5 MB e 13.700 vagas, cada uma com `<lastmod>`;
  - `lastmod` nos ultimos 60 dias deixa 7.417; nivel de entrada no slug, 909;
    tecnologia no titulo do slug, 42. Sem o ultimo corte seriam ~25 minutos de
    requisicao; com ele, 1m30, e 5 vagas chegam ao aviso;
  - a pagina e Nuxt renderizada no servidor, com a vaga inteira em
    `window.__NUXT__` -- um literal JavaScript, nao JSON (ver `_ler_nuxt`);
  - `workTypes` veio preenchido em 40 de 40 paginas, sempre com um valor so:
    34 `presencial`, 4 `remoto`, 2 `hibrida`;
  - `levelOfInterests` NAO e confiavel: "Analista" numa vaga Junior,
    "Especialista" em "Tecnico de informatica JR", "Consultor" em "Programador
    Web - Trainee". Fica fora de `job.seniority`, e o titulo decide.

Os termos de uso que o proprio site linka (`help.abler.com.br/.../termos-de-uso`
e `.../termo-de-consentimento`) respondem 404 nessa data; a politica de
privacidade so trata de dados de candidatos. Nada encontrado proibe reproduzir
a vaga.

**Empresa oculta:** quando a empresa marca `hideCompany`, o nome dela vem no
dado mesmo assim. O coletor o descarta: a escolha de nao aparecer e dela.
...
_URL_VAGA_RE = re.compile(
    r"<loc>(https://candidatos\.abler\.com\.br/vagas/([^<]+))</loc>\s*"
    r"(?:<lastmod>([^<]*)</lastmod>)?"
)
...
# Teto de seguranca, como na GeekHunter. Hoje o filtro devolve 42.
MAX_VAGAS = 120
...
class AblerSource(JobSource):
    ...
    def _candidatas(self, sitemap: str, hoje: date | None = None) -> list[str]:
        """URLs que o pipeline nao descartaria pelo que o slug ja diz.

        As funcoes sao as do pipeline, de proposito. A idade usa o `lastmod`:
        ele nunca foi anterior ao `publishedAt` nas 40 paginas medidas, entao
        `lastmod` velho implica vaga velha. O pipeline reaplica o filtro de
        idade com a data de publicacao.
        """
        entrada, clf = default_filter(), default_classifier()
        urls = []
        for url, slug, lastmod in _URL_VAGA_RE.findall(sitemap):
            if self.settings.dias_max > 0:
                idade = dias_desde(lastmod, hoje)
                if idade is not None and idade > self.settings.dias_max:
                    continue
    ...
    def _parse(self, html: str, url: str) -> Job | None:
        ...
        return Job(
            ...
            url=url,
            description=vaga.get("description") or "",
            location=local,
