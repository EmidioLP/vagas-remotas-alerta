...
def test_gerente_rotulado_de_intern_e_descartado_no_pre_filtro():
    ...
    ficaram = fonte._pre_filtrar([fonte._parse(GERENTE_COMO_INTERN)])
    assert ficaram == []


def test_vaga_junior_remota_passa_o_pre_filtro():
    fonte = _fonte(_Sessao([]))
...
def test_presencial_no_rn_passa_o_pre_filtro():
    """Modalidade presencial não elimina: o que vale é o local."""
    fonte = _fonte(_Sessao([]))
    job = fonte._parse({**ITEM, "workMode": "Onsite",
                        "location": "Natal, Rio Grande do Norte, BR",
...
def test_fetch_busca_descricao_so_das_candidatas():
    ...
    itens = [ITEM, GERENTE_COMO_INTERN,
             {**ITEM, "id": "presencial-sp", "workMode": "Onsite",
              "location": "Curitiba, PR, BR"}]
    sessao = _Sessao([_envelope(itens)],
                     detalhes={ITEM["id"]: {"description": "Vaga de Python."}})
    jobs = _fonte(sessao).fetch(["desenvolvedor junior"])

    assert [j.external_id for j in jobs] == [ITEM["id"]]
    detalhes_pedidos = [u for u, p in sessao.urls if p is None]
    assert len(detalhes_pedidos) == 1, "só a candidata deve custar requisição"
