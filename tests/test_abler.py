...
from scraper.sources.abler import MAX_VAGAS, AblerSource, _ler_nuxt
...
def _pagina(vaga: str, parametros: str = "a,b,c,d,e",
    ...
    """Recorte real da pagina: o `__NUXT__` com a vaga em state.candidate.vacancy."""
    return (
...
# Vaga 393184 (AnkaTech): remota, empresa visivel, nivel "Junior".
VAGA_REMOTA = (
...
# Vaga 391547: presencial em Natal, empresa oculta, nivel "Assistente".
VAGA_OCULTA_NATAL = (
...
def _source(sessao=None, **ajustes):
    return AblerSource(session=sessao, settings=Settings(**ajustes))


def test_pre_filtro_pelo_slug_e_pelo_lastmod():
    urls = _source()._candidatas(SITEMAP, hoje=HOJE)
    # Fica so a junior de tech recente: advogado nao e tech, Android e
    # senior, e a full stack jr e de 2025.
    assert urls == ["https://candidatos.abler.com.br/vagas/desenvolvedor-fullstack-junior-637999"]


def test_dias_max_zero_desliga_o_corte_por_idade():
    urls = _source(dias_max=0)._candidatas(SITEMAP, hoje=HOJE)
    assert "https://candidatos.abler.com.br/vagas/desenvolvedor-full-stack-jr-56946" in urls
...
def test_parse_mapeia_os_campos():
    ...
    assert job.external_id == "393184"  # o da pagina, nao o 637999 do slug
...
def test_empresa_oculta_nao_aparece():
    job = _source()._parse(_pagina(VAGA_OCULTA_NATAL), "u")
    assert job.company == ""
...
def test_variaveis_do_nuxt_mudam_de_pagina_para_pagina():
    # A mesma vaga, com as letras trocadas: `d` agora e false e `b` e true.
...
def test_sitemap_indisponivel_nao_derruba_a_coleta():
    assert _source(_Sessao({})).fetch([]) == []


def test_fetch_vai_do_sitemap_a_pagina(monkeypatch):
    url = "https://candidatos.abler.com.br/vagas/desenvolvedor-fullstack-junior-637999"
    fonte = _source(_Sessao({url: _pagina(VAGA_REMOTA)}))
    monkeypatch.setattr(fonte, "_candidatas", lambda sitemap: [url])
    fonte.session.paginas["https://candidatos.abler.com.br/sitemap.xml"] = SITEMAP
...
def test_teto_de_seguranca_existe():
    assert 0 < MAX_VAGAS <= 200


def test_numero_sem_zero_antes_do_ponto():
    # `salaryValue:.01` veio numa pagina real (vaga 391212) e quebrava a leitura.
