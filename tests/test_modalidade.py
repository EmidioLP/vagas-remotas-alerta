...
from scraper.locais import resolver, serve
...
# No filtro de remotas: so a inferencia de Remoto muda o que chega ao Discord.

@pytest.mark.parametrize("descricao,passa", [
    ("Modelo de trabalho: 100% remoto.", True),
    ("Prestar suporte remoto aos usuários.", False),
    ("Trabalho remoto ou híbrido, à escolha.", False),
    ("Modelo híbrido em São Paulo.", False),
    ("", False),
])
def test_filtro_de_remotas_usa_a_modalidade_inferida(descricao, passa):
    job = Job(source="vagas", external_id="1", title="Dev Jr", description=descricao,
              location="São Paulo / SP", workplace_type="Não informado")
    completar_modalidade([job])
    assert serve(job, resolver(["rn", "fortaleza"])) is passa


def test_produto_de_trabalho_remoto_nao_e_modalidade():
    # Recorte real (LinkedIn, Enghouse): fala do produto da empresa.
    descricao = ("Empresa internacional especializada em soluções de software "
                 "empresarial para o futuro do trabalho remoto, computação visual avançada.")
    assert inferir_modalidade("Junior Software Developer (Android)", descricao) == "Não informado"
